//code 1 to adding the colums elements 
#include<iostream>
#include<cstdlib>
#include<iomanip>
...
void SwapArray(int array[row][colm],int array2[row][colm]) {
	for (int i = 0;i<row;i++) {
		for (int j = 0;j<colm;j++) {
...
int main() {
	...
	PrintArray(array1);
	cout << "\n\nswaped array is : " << endl;
	int arraySwaped[row][colm] = {0};
	SwapArray(array,arraySwaped);
	PrintArrayRandomNums(arraySwaped); cout << endl;
	ArraySumArray(arraySwaped,array1);
	PrintArray(array1);
}
